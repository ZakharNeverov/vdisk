...
struct vdisk_dev {
    ...
    spinlock_t lock;           /* Блокировка (если потребуется синхронизация) */
...
/* Определяем глобальный tag_set для blk-mq */
static struct blk_mq_tag_set vdisk_tag_set;

/*
 * Функция-обработчик запросов в очереди blk-mq.
 * Использует интерфейс blk-mq для получения запроса и обработки сегментов.
 */
                                     ...
                                     const struct blk_mq_queue_data *bd)
{
    ...
    /* Проверяем, чтобы запрос не выходил за пределы устройства */
    if (offset + blk_rq_cur_bytes(req) > dev->size) {
        printk(KERN_ERR "vdisk: Запрос за пределами устройства. offset=%lu, size=%u\n",
               offset, dev->size);
...

    {
        ...
        rq_for_each_segment(bvec, req, iter) {
            ...
            /* В зависимости от направления запроса копируем данные */
            if (dir == READ)
...
/* Функция открытия блочного устройства */
static int vdisk_open(struct block_device *bdev, fmode_t mode)
{
    return 0;
}

/* Функция закрытия блочного устройства */
 ...
 * Обратите внимание: сигнатура для блочных устройств:
                       ...
                       unsigned int cmd, unsigned long arg)
{
    ...
    case VDISK_IOCTL_SAVE_IMAGE:
        ...
        filename[255] = '\0';
        printk(KERN_INFO "vdisk: Saving into an image file: %s\n", filename);
    ...
    case VDISK_IOCTL_LOAD_IMAGE:
        ...
        filename[255] = '\0';
        printk(KERN_INFO "vdisk: restoring from image file: %s\n", filename);
...
/* Инициализация драйвера (модуль) */
static int __init vdisk_init(void)
{
    int ret;

    /* Выделяем память под наше устройство */
    ...
    /* Выделяем буфер для данных (виртуальный диск) */
    ...
    /* Регистрируем блочное устройство, получая главный номер */
    ...
    /* Выделяем и настраиваем структуру описания устройства */
    ...
    printk(KERN_INFO "vdisk: driver loaded. major = %d, size = %u bytes\n",
...
static void __exit vdisk_exit(void)
{
    ...
    vfree(device->data);
    kfree(device);
    printk(KERN_INFO "vdisk: driver unloaded\n");
}
...
MODULE_DESCRIPTION("Виртуальный диск с сохранением/восстановлением образа (ядро 6.1.0-31-amd64)");
